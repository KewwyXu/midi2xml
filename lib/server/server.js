...
import { parseMIDIToXML } from "../index.js";
...
if (saveFile) {
    ...
    app.post("/api/parseMIDIToXML", upload.single("midiFile"), (async (req, res) => {
        try {
            ...
            const xml = await parseMIDIToXML(req.file.path, xmlPath, req.file.originalname.replace(new RegExp(".midi?"), ""), jsonPath);
            // 返回转换后的 XML 文件
            res.status(200).json({
                message: "转换成功",
                xmlPath: xmlPath,
    ...
    app.post("/api/parseMIDIToXML", upload.single("midiFile"), (async (req, res) => {
        try {
            ...
            const xml = await parseMIDIToXML(req.file.buffer.buffer, undefined, req.file.originalname.replace(new RegExp(".midi?"), ""));
            res.status(200).json({
                message: "转换成功",
                xml: xml,
            });
