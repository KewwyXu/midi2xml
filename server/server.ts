...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(
      null,
      `${path.parse(file.originalname).name}-${uniqueSuffix}${path.parse(file.originalname).ext}`,
    );
  },
});
...
app.post("/parseMIDIToXML", upload.single("midiFile"), (async (req, res) => {
  try {
    ...
    const xmlPath = path.join(
      xmlsDir,
      `${req.file.filename.replace(new RegExp(".midi?"), "")}.xml`,
    );
    const jsonPath = path.join(
      jsonsDir,
      `${req.file.filename.replace(new RegExp(".midi?"), "")}.json`,
    );
