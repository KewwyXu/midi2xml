...
describe("parseMIDIToXML from midi path", () => {
  test("success", async () => {
    expect(
      await parseMIDIToXML(
        "C:\\Users\\kewwy\\RiderProjects\\midisheetmusic\\songs\\Bach__Invention_No._13.mid",
        undefined,
        undefined,
        "./tests/midiData.json",
      ),
  ...
  test("fail to parse midi", async () => {
    await expect(
      parseMIDIToXML("C:\\code\\@kiwi\\midi2xml\\sample-midi\\sample.mid", ""),
    ).rejects.toThrow();
  });
});
...
describe("parseMIDIToXML from midi file", () => {
  test("success", async () => {
    ...
    await expect(
      parseMIDIToXML(
        file,
        path.resolve(__dirname, "../sample-midi/sample.xml"),
      ),
  ...
  test("fail to read midi", async () => {
    const file = null;
    const xmlPath = require("path").resolve(
      __dirname,
      "../sample-midi/sample.xml",
    );
    try {
  ...
  test("fail to parse midi", async () => {
    // 构造一个无效的 File
    const file = new File([new Uint8Array([0, 1, 2, 3])], "wrongMIDI.mid", {
      type: "audio/midi",
    });
    const xmlPath = require("path").resolve(
      __dirname,
  ...
  test("fail when xmlPath not found", async () => {
    ...
    const file = new File([buffer], "sample.mid", { type: "audio/midi" });
    // 传递不存在的xmlPath
    const notExistPath = path.resolve(
      __dirname,
      "../sample-midi/not-exist-folder/sample.xml",
    );
    try {
